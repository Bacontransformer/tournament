  ...
  export default {
    ...
    methods: {
      getStatusLabel(status) {
        return this.statusOptions.find(item => item.value === String(status))?.label || '未知'
      },
      formatTime(time) {
      ...
      async fetchData() {
        ...
        try {
          const apiMap = {
            '0': '/api/common/match-b/un-start/page',
            '1': '/api/common/match-b/doing/page',
            '2': '/api/common/match-b/end/page'
          }
          
          const { data: response } = await axios.post(apiMap[this.queryForm.status], {
          ...
          this.$message.error('数据加载失败: ' + (error.response?.data?.message || ''))
        } finally {
          this.loading = false
        }
      },
